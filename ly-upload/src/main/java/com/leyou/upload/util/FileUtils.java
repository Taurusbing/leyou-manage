...
public class FileUtils {
	...
	public static Map<String, Object> fileUpload(MultipartFile[] myFiles, String targetDir, String imgDir,
		...
		try {
			for (MultipartFile myFile : myFiles) {
				...
				if ("true".equals(map1.get(flag).toString())) {
					String imgPath = "/"; 
					imgPath += map1.get(data).toString();
					String imgfileName = map1.get(fileName).toString();
			...
			if (originalFilename.length() > 0) {
				map.put(data, originalFilename.substring(0, originalFilename.length() - 1)); // 上传成功的所有的图片地址的路径
			} else {
				map.put(data, originalFilename); // 上传成功的所有的图片地址的路径
			}
	...
	public static Map<String, Object> fileUpload(MultipartFile myFile, String targetDir, String imgDir,
		...
		if (myFile.isEmpty()) {
			...
			map.put(message, "请求不包含multipart/form-data流");
		} else if (maxSize < contentLength) {
			System.out.println("上传文件大小超出文件最大大小");
			map.put(message, "上传文件大小超出文件最大大小[" + maxSize + "]");
		} else if (!ServletFileUpload.isMultipartContent(request)) {
			map.put(message, "请选择文件");
		} else if (!uploadDir.isDirectory()) {// 检查目录
			map.put(message, "上传目录[" + savePath + "]不存在");
		} else if (!uploadDir.canWrite()) {
			map.put(message, "上传目录[" + savePath + "]没有写权限");
		} else {
			...
			if (rename == 0) {
				...
				map.put(data, targetDir + imgDir + originalFilename);
				...
				org.apache.commons.io.FileUtils.copyInputStreamToFile(myFile.getInputStream(),
						new File(targetDir + imgDir, originalFilename));
				map.put("totalSize", getSize((double) myFile.getInputStream().available()));
				map.put(data, targetDir.split("/")[1] + imgDir + originalFilename);
				map.put(fileName, originalFilename);
				map.put("originalfilename", myFile.getOriginalFilename());
				map.put(flag, true);
			}
...
//			System.out.println(DateUtil.formatLongToStr(file.lastModified(), "yyyy-MM-dd HH:mm:ss"));
//		}
//	}
